...
import main.entities.characters.Enemy;
...
public class Bullet {
    ...
    public void update(List<Structure> structures, Player player) {
        if (active) {
            // Mover la bala
            switch (direction) {
                case 0: // Derecha
            ...
            if (x < Window.limiteJugadorIzquierda || x > Window.limiteJugadorDerecha + Window.bloqueTam*0.5 || y < Window.limiteJugadorArriba || y > Window.limiteJugadorAbajo + Window.bloqueTam*0.5) {
                deactivate();
            }
    ...
    public boolean collidesWithEnemy(Enemy enemigo) {
        return x < enemigo.getX() + enemigo.getWidth() &&
               x + width > enemigo.getX() &&
               y < enemigo.getY() + enemigo.getHeight() &&
               y + height > enemigo.getY();
    }
